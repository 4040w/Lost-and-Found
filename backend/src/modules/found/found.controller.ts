...
export class FoundController {
  ...
  @Delete(':id')
  @ApiOperation({ summary: '删除自己发布的招领信息' })
  @Auth()
  async delete(@CurrentUser() user: UserModel, @Param('id', ParseObjectIdPipe) id: string) {
    return this.foundService.delete(user, id)
  }
  ...
  async foundLostById(@Param('id', ParseObjectIdPipe) id: string) {
